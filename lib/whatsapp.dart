...
class WhatsApp {
  ...
  setup({accessToken, int? fromNumberId}) {
    ...
    _headers = {"Content-Type": "application/json", "Authorization": "Bearer $_accessToken"};
  }
  ...
  Future messagesTemplateText({int? to, String? templateName, String? languageCode, List? components}) async {
  ...
  Future messagesLocation({to, longitude, latitude, name, address}) async {
    ...
    Map data = {
      ...
      "to": to,
      "type": "location",
      "location": {"longitude": longitude, "latitude": latitude, "name": name, "address": address}
    };
  ...
  Future messagesMediaByLink({to, mediaType, mediaLink, caption}) async {
    ...
    var response = await http.post(Uri.parse(url), headers: _headers, body: body);
    try {
      return json.decode(response.body);
    } catch (e) {
      return response.body;
  ...
  Future getWhatsAppBusinessAccounts({inputToken}) async {
    var url = 'https://graph.facebook.com/v18.0/debug_token?input_token=$inputToken';
  ...
  Future getWhatsAppBusinessAccountsList({accountId}) async {
    ...
    var url = 'https://graph.facebook.com/v18.0/$parseAccountId/client_whatsapp_business_accounts';
  ...
  uploadMedia({required mediaFile, mediaType, mediaName}) async {
    var uri = Uri.parse('https://graph.facebook.com/v18.0/$_fromNumberId/media');

    var request = http.MultipartRequest('POST', uri);
    request.headers.addAll(_headers!);
    request.fields['messaging_product'] = 'whatsapp';
    request.files.add(http.MultipartFile.fromBytes('file', File(mediaFile.path).readAsBytesSync(), filename: mediaName, contentType: mediaType));
  ...
  Future updateProfile({businessAddress, businessDescription, businessIndustry, businessAbout, businessEmail, required List businessWebsites, businessProfileId}) async {
    var url = 'https://graph.facebook.com/v18.0/$_fromNumberId/whatsapp_business_profile';
